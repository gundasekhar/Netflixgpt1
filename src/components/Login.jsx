...
const Login = () => {
  ...
  return (
    <div className='relative'>
      ...
      <form onSubmit={(e) => e.preventDefault()} className='bg-black/80 absolute top-40 mx-auto right-0 left-0 w-75 p-8 text-white bg-opacity-80'>
        ...
        <input ref={email} type='text' placeholder='Email Address' className=' bg-gray-700 p-1.5 mt-4 w-full text-white'/>
        <input ref={password} type='passowrd' placeholder='Password' className='bg-gray-700 p-1.5 mt-4 w-full text-white'/>
        <p className='text-red-500 my-2'>{errMsg}</p>
        <button onClick={handleButtonClick} className='cursor-pointer bg-red-700 w-full p-2 mt-5'>{!isSignIn ? "Sign Up" : "Sign In"}</button>
